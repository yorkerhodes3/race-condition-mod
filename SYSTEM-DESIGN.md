...
The **Digital Twin Systems Framework** (DTSF) is a TypeScript/Node runtime that
hosts "twins" — behavioral clones of external HTTP services, each defined by an
OpenAPI contract + a `BaseBehaviorPack` + seeded, snapshot-able state + fault
profiles. It is runnable locally via **"Start DTSF server"** (`npm start`,
serving `http://localhost:8080`, discovering packs under `twins/packs/*/twin.yaml`).
...
2. **Later:** expose RC's NDJSON via DTSF's **Replay** feature over SSE
   (recordings = trajectory source, DTSF snapshots = restore points).
