...
A [Digital Twin Systems Framework](https://) twin that clones the **Race
...
```powershell
Copy-Item -Recurse `
  "C:\Dev\race-condition-mod\dtsf\packs\race-condition" `
  "C:\Dev\Digital Twin Systems Framework\twins\packs\race-condition"
```
...
by changing `RUNNER_MODEL` — no cloud, no code changes. See the framework's
`OLLAMA-SPEC.md`.
